...
4. **`/ai/DEV_ENVIRONMENT.md`** — Ubuntu-native conventions (ADR-026).
...
- ADR-001 (WSL-native dev, no Docker for app code) — was originally kept with a docker-compose carve-out during Stage 1; later **superseded by ADR-026 (native Ubuntu 26 dev environment)**. The principle of no-Docker-for-the-application is preserved in ADR-026; the docker-compose carve-out for supporting services is also preserved.
...
6. **P0-T6: Add `scripts/setup.sh`.** Generates `.env` files with strong randoms, runs `docker-compose up -d`, prints next steps. The script must be idempotent (re-runnable without breaking state) and refuse to overwrite existing `.env` without `--force`. (PowerShell variant `setup.ps1` was originally in scope; dropped per ADR-026 — Ubuntu-only target.)
