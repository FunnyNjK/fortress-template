...
**Latest **`workflow`** **`CI`** (**`name`** **`CI`****)** **:** **`gh`** **`captures`** **`DONE_LOG`** **`CHAT`** **`JSON`** (**`databaseId`** **+ **`headSha`** **)** **`—`** **`e.g.** **`9ad2090`**, **`25611241774`**, **`success`** **(**`prior`** **`to`** **`the`** **`next`** **`planning`** **`push`** **)**.** **`Locally`** **:** **`npx pnpm@10.33.4`** **`lint`** **`typecheck`** **`test`** **`build`**; **`pnpm --filter api test`**; **`pnpm --filter api test:integration`**; **`pnpm audit --audit-level=high`** (**1 moderate**, below **`high`** gate). **`CHAT`** **routine**: **`git fetch`**; **`gh`** (**`workflow`** **`CI`** **`list`** **`+`** **`watch`****)**; **skip** **`full`** **`local`** **`suite`** **`unless`** **`CI`** **`regresses`** **.**
