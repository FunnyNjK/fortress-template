...
- **P2-T6** — **`HealthModule`** (`apps/api/src/health/`): **`GET /healthz`** (**`Public`**, **`SkipRateLimit`**, **`process.uptime`**, **`FORTRESS_API_VERSION`** via **`@fortress/types`**), **`GET /readyz`** (**Drizzle `SELECT 1`**, Redis **`ping`**, HTTP 503 **`{ status, checks }`**); **`SkipRateLimit` decorator**, **`SKIP_RATE_LIMIT_KEY`** in **`RateLimitGuard`**; **`AppModule`** **`HealthModule`**; **`apps/api`** depends **`@fortress/types`**; **`test/integration/`** (**security-chain**, **auth-flow**, **readiness**, **`index.test.ts`** incl. **`/healthz`** not rate limited after 121 hits); **`vitest.integration.config.ts`**; root **`pnpm --filter api test`** excludes **`test/integration/**`**; **`test:integration`** + CI **`api-integration`** (**compose `.env`**, **`postgres`/`redis` --wait**, **`db:migrate`**); **`README.md`**; superseded **`test/app.bootstrap.test.ts`**.

- CHAT_END (2026-05-09): **`feat(api):`** **P2-T6** (see branch tip **`git`** **`log`** **`-1`**); Ritual deltas in **`DONE_LOG`** / **`TASKS`** / **`HANDOFF`** / **`CURRENT_STATE`** / **`TESTING`** / manifests; **`npx pnpm@10.33.4`** `lint` `typecheck` `test` `build`; **`pnpm audit --audit-level=high`** (**1 moderate**); **`python3`** `yaml.safe_load` **`ci.yml`**. **`pnpm --filter api test:integration`** with **`CI=true`** requires Compose Redis credentials matching **`REDIS_URL`** (CI **`redis://:test@...`**).
